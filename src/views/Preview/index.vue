        ...
        <div class="body-container preview-page__body">
            ...
            <div class="mb-15px">
                <p class="flex justify-end">
                    编号：【<span class="text-underline">{{ formData.data.contractCode || '' }}</span
                    >】
                </p>
            </div>
            <div class="mb-24px">
                <p>
                    甲方（需方）：<span class="text-underline">{{ formData.data.enterpriseName || '' }}</span>
                </p>
                <p>甲方（公章/合同章）：</p>
                <p>
                    日期：<span class="text-underline">{{ formData.data.contractTime || '' }}</span>
                </p>
            </div>
            <div class="mb-24px">
            ...
            <div class="mb-50px">
                <p class="indent-lv1">
                    甲方根据与中国联合网络通信有限公司【<span class="text-underline">{{
                        formData.data.unicomCity || ''
                    }}</span
                    >】分公司（以下简称“【<span class="text-underline">{{ formData.data.unicomCity || '' }}</span
                ...
                <dl class="flex items-start indent-lv2">
                    ...
                    <dd>
                        <p>
                            甲方与【<span class="text-underline">{{ formData.data.unicomCity || '' }}</span
                ...
                <dl class="flex items-start indent-lv2">
                    ...
                    <dd>
                        <p>
                            甲方授权【<span class="text-underline">{{ formData.data.name || '' }}</span
                            >】为履行本合同代理人，有权接受和签发与本协议相关的各种文件，包括并不限于附件确认、收货单据等。代理人具体信息：姓名【<span
                                class="text-underline"
                                >{{ formData.data.name || '' }}</span
                            >】，身份证号码【<span class="text-underline">{{ formData.data.idcard || '' }}</span
                            >】， 联系电话【<span class="text-underline">{{ formData.data.phone || '' }}</span
                            >】。
                        </p>
                    </dd>
                </dl>
                ...
                <table class="pure-table">
                    <thead>
                        <tr>
                            <th colspan="6" class="font-bold">
                                业务配套服务包采购清单-第<span class="text-underline">{{
                                    formData.data.batch || ''
                                }}</span
                                >批
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="flex items-center justify-center">
                                    <div>
                                        合计台数：<span class="text-underline">{{ formData.data.countSum || '' }}</span>
                                    </div>
                                    <div class="mx-2">
                                        合计首付款：<span class="text-underline">{{
                                            formData.data.firstPaymentSum || ''
                                        }}</span>
                                    </div>
                                    <div>
                        ...
                        <tr class="pure-table-odd">
                            <td>收货单位</td>
                            <td colspan="5">{{ formData.data.enterpriseName || '' }}</td>
                        </tr>
                        <tr>
                            <td>收货人</td>
                            <td>{{ formData.data.consigneeName || '' }}</td>
                            <td>联系电话</td>
                            <td colspan="3">{{ formData.data.consigneePhone || '' }}</td>
                        </tr>
                        <tr class="pure-table-odd">
                            <td>收货地址</td>
                            <td colspan="5">{{ formData.data.consigneeAddress || '' }}</td>
                        </tr>
                        <tr>
                            <td colspan="6">
        ...
        try {
            ...
            if (res && !isEmpty(res.data)) {
                ...
                res.data.buyList.forEach(item => {
                    if (buyList[item.period]) {
                        buyList[item.period].push(item)
                    } else {
                        buyListIndex.value.push(item.period)
                        buyList[item.period] = [item]
                    }
                })
